<#
.DESCRIPTION
Get-SecuronixEntityThreatModel prepares API parameters and queries the Securonix API for a list of all threats violated in a time range.
...
.INPUTS
None. You cannot pipe objects to Get-SecuronixEntityThreatModel

.OUTPUTS
System.String. Get-SecuronixEntityThreatModel returns the API response. The API will respond with a JSON object for valid requests.

.EXAMPLE
PS> Get-SecuronixEntityThreatModel -Url 'DunderMifflin.securonix.com/Snypr' -Token '12345678-90AB-CDEF-1234-567890ABCDEF' -DocumentId '2^~A^~7|NULL|AW2385^~C^~1^~EP^~66)'

.EXAMPLE
PS> Get-SecuronixEntityThreatModel -Url 'DunderMifflin.securonix.com/Snypr' -Token '12345678-90AB-CDEF-1234-567890ABCDEF' -DocumentId '2^~A^~7|NULL|AW2385^~C^~1^~EP^~66)' -TenantName 'PA-Scranton'
